...
#region Getters/Setters:
	...
	public override IEnumerator Routine(DestinoBoss boss)
	{
		...
		IEnumerator rotationRoutine = VCoroutines.RotateAroundTransform(deck.ToArray<DestinoCard>(), boss.transform, Vector3.up, orbitSpeed, orbitRadius, wait.MoveNext); 
	...
	private IEnumerator PositionCardsAroundDestino(Boss boss)
	{
		...
		{
			spawnPositions[i] = boss.transform.position + (currentRotation * (Vector3.right * orbitRadius));
	...
	private IEnumerator PresentSelectedCard(DestinoBoss boss)
	{
		...
		while(t < 1.0f)
		{
			selectedCard.transform.position = Vector3.Lerp(selectedCard.transform.position, selectedCardPoint, t);
			t += (Time.deltaTime * inverseDuration);
			yield return null;
		}
	...
	private IEnumerator ShuffleDeck()
	{
		...
		while(t < (1.0f + Mathf.Epsilon))
		{
			foreach(DestinoCard card in deck)
			{
				card.transform.position = Vector3.Lerp(card.transform.position, deckPoint, t);
			}
