...
Shader "DEC/Cutout Wind/Cutout Wind Translucency"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float floatswitch168_g39828( int m_switch, float m_Off, float m_ObjectSpace, float m_WorldSpace, float m_VertexColor, float m_VertexNormal )
			{
				if(m_switch ==0)
					return m_Off;
				else if(m_switch ==1)
			...
			float4 float4switch313_g39828( int m_switch, float4 m_Off, float4 m_Active, float4 m_ActiveInverted )
			{
				if(m_switch ==0)
					return m_Off;
				else if(m_switch ==1)
			...
			float4 float4switch364_g39828( int m_switch, float4 m_Off, float4 m_ObjectSpace, float4 m_WorldSpace, float4 m_VertexColor, float4 m_VertexNormal )
			{
				if(m_switch ==0)
					return m_Off;
				else if(m_switch ==1)
			...
			float3 float3switch56_g39838( int m_switch, float3 m_Default, float3 m_Flip, float3 m_Mirror )
			{
				if(m_switch ==0)
					return m_Default;
				else if(m_switch ==1)
			...
			float3 float3switch37_g39838( int m_switch, float3 m_Default, float3 m_Flip, float3 m_Mirror )
			{
				if(m_switch ==0)
					return m_Default;
				else if(m_switch ==1)
			...
			float floatswitch1287_g39777( int m_switch, float m_Off, float m_Active )
			{
				if(m_switch ==0)
					return m_Off;
				else if(m_switch ==1)
			...
			float4 float4switch189_g39778( int m_switch, float4 m_Smoothness, float4 m_Roughness )
			{
				if(m_switch ==0)
					return m_Smoothness;
				else if(m_switch ==1)
			...
			float4 float4switch188_g39778( int m_switch, float4 m_Standard, float4 m_Geometric, float4 m_GeometricFade, float4 m_Texture )
			{
				if(m_switch ==0)
					return m_Standard;
				else if(m_switch ==1)
			...
			float4 float4switch72_g39808( int m_switch, float4 m_Off, float4 m_Active )
			{
				if(m_switch ==0)
					return m_Off;
				else if(m_switch ==1)
			...
			float4 float4switch50_g39808( int m_switch, float4 m_Off, float4 m_Active )
			{
				if(m_switch ==0)
					return m_Off;
				else if(m_switch ==1)
			...
			VertexOutput VertexFunction( VertexInput v  )
			{
				...
				float3 temp_output_1234_0_g39777 = localfloat3switch3050_g3466;
				...
				#endif
				float3 vertexValue = temp_output_1234_0_g39777;
				#ifdef ASE_ABSOLUTE_VERTEX_POS
						...
						, FRONT_FACE_TYPE ase_vface : FRONT_FACE_SEMANTIC ) : SV_Target
			{
				...
				int m_switch364_g39828 = _ColorShift_Mode;
				float2 appendResult168_g39777 = (float2(_TilingX , _TilingY));
				float2 appendResult167_g39777 = (float2(_OffsetX , _OffsetY));
				float2 texCoord2_g39816 = IN.ase_texcoord7.xy * appendResult168_g39777 + appendResult167_g39777;
				float2 appendResult6_g39816 = (float2(texCoord2_g39816.x , texCoord2_g39816.y));
				float2 OUT_UV213_g39777 = appendResult6_g39816;
				float2 UV40_g39822 = OUT_UV213_g39777;
				float4 tex2DNode63_g39822 = SAMPLE_TEXTURE2D( _MainTex, sampler_trilinear_repeat, UV40_g39822 );
				float4 ALBEDO_RGBA1381_g39777 = tex2DNode63_g39822;
				float3 temp_output_3_0_g39777 = ( (_Color).rgb * (ALBEDO_RGBA1381_g39777).rgb * _Brightness );
				float3 temp_output_134_0_g39828 = temp_output_3_0_g39777;
				float4 m_Off364_g39828 = float4( temp_output_134_0_g39828 , 0.0 );
				int m_switch313_g39828 = _ColorShift_MaskMode;
				int MODE176_g39828 = _ColorShift_Mode;
				int m_switch168_g39828 = MODE176_g39828;
				float m_Off168_g39828 = 0.0;
				float m_ObjectSpace168_g39828 = ( _ColorShift_NoiseScale / 3 );
				float3 p1_g39829 = ( WorldPosition * _ColorShift_WorldSpaceNoiseShift );
				float localSimpleNoise3D1_g39829 = SimpleNoise3D( p1_g39829 );
				float4 transform374_g39828 = mul(GetObjectToWorldMatrix(),IN.ase_texcoord8);
				float m_WorldSpace168_g39828 = ( ( localSimpleNoise3D1_g39829 / _ColorShift_NoiseScale ) - ( ( (transform374_g39828).w - _ColorShift_WorldSpaceOffset ) / _ColorShift_WorldSpaceDistance ) );
				float m_VertexColor168_g39828 = ( IN.ase_color.g - 0.5 );
				float m_VertexNormal168_g39828 = ( IN.ase_normal.y - 0.5 );
				float localfloatswitch168_g39828 = floatswitch168_g39828( m_switch168_g39828 , m_Off168_g39828 , m_ObjectSpace168_g39828 , m_WorldSpace168_g39828 , m_VertexColor168_g39828 , m_VertexNormal168_g39828 );
				float temp_output_112_0_g39828 = sin( ( _ColorShift_NoiseScale * PI ) );
				float3 ALBEDO_IN136_g39828 = temp_output_134_0_g39828;
				float2 appendResult120_g39828 = (float2(( (0.3 + (( 1.0 - temp_output_112_0_g39828 ) - 0.0) * (1.0 - 0.3) / (1.0 - 0.0)) * float4( ALBEDO_IN136_g39828 , 0.0 ).x ) , 0.0));
				float2 RGB146_g39828 = appendResult120_g39828;
				float3 hsvTorgb122_g39828 = RGBToHSV( float3( RGB146_g39828 ,  0.0 ) );
				float VALUE219_g39828 = temp_output_112_0_g39828;
				float3 hsvTorgb126_g39828 = HSVToRGB( float3(( ( saturate( localfloatswitch168_g39828 ) * _ColorShift_ShiftVariation ) + _ColorShift_ShiftVariationRGB + hsvTorgb122_g39828 ).x,( _ColorShift_ShiftSaturation * hsvTorgb122_g39828.y ),( hsvTorgb122_g39828.z + ( VALUE219_g39828 / 40 ) )) );
				float4 m_Off313_g39828 = float4( hsvTorgb126_g39828 , 0.0 );
				float2 uv_ColorShift_MaskMap = IN.ase_texcoord7.xy * _ColorShift_MaskMap_ST.xy + _ColorShift_MaskMap_ST.zw;
				float4 transform376_g39828 = mul(GetObjectToWorldMatrix(),float4( IN.ase_texcoord8.xyz , 0.0 ));
				float4 temp_output_337_0_g39828 = saturate( ( 1.0 - ( ( SAMPLE_TEXTURE2D( _ColorShift_MaskMap, sampler_trilinear_repeat, uv_ColorShift_MaskMap ) * transform376_g39828 ) / max( _ColorShift_MaskFuzziness , 1E-05 ) ) ) );
				float3 lerpResult314_g39828 = lerp( hsvTorgb126_g39828 , ALBEDO_IN136_g39828 , temp_output_337_0_g39828.rgb);
				float4 m_Active313_g39828 = float4( lerpResult314_g39828 , 0.0 );
				float3 lerpResult311_g39828 = lerp( ALBEDO_IN136_g39828 , hsvTorgb126_g39828 , temp_output_337_0_g39828.rgb);
				float4 m_ActiveInverted313_g39828 = float4( lerpResult311_g39828 , 0.0 );
				float4 localfloat4switch313_g39828 = float4switch313_g39828( m_switch313_g39828 , m_Off313_g39828 , m_Active313_g39828 , m_ActiveInverted313_g39828 );
				float4 lerpResult297_g39828 = lerp( float4( temp_output_134_0_g39828 , 0.0 ) , localfloat4switch313_g39828 , _ColorShift_ShiftInfluence);
				float4 m_ObjectSpace364_g39828 = lerpResult297_g39828;
				float4 m_WorldSpace364_g39828 = lerpResult297_g39828;
				float4 m_VertexColor364_g39828 = lerpResult297_g39828;
				float4 m_VertexNormal364_g39828 = lerpResult297_g39828;
				float4 localfloat4switch364_g39828 = float4switch364_g39828( m_switch364_g39828 , m_Off364_g39828 , m_ObjectSpace364_g39828 , m_WorldSpace364_g39828 , m_VertexColor364_g39828 , m_VertexNormal364_g39828 );
				
				float4 NORMAL_RGB1382_g39777 = SAMPLE_TEXTURE2D( _BumpMap, sampler_trilinear_repeat, UV40_g39822 );
				float4 temp_output_1_0_g39820 = NORMAL_RGB1382_g39777;
				float temp_output_8_0_g39820 = _NormalStrength;
				float3 unpack52_g39820 = UnpackNormalScale( temp_output_1_0_g39820, temp_output_8_0_g39820 );
				unpack52_g39820.z = lerp( 1, unpack52_g39820.z, saturate(temp_output_8_0_g39820) );
				float3 temp_output_1519_59_g39777 = unpack52_g39820;
				float3 NORMAL_IN42_g39838 = temp_output_1519_59_g39777;
				int m_switch56_g39838 = _NormalMode;
				float3 m_Default56_g39838 = float3(1,1,1);
				float3 m_Flip56_g39838 = float3(-1,-1,-1);
				float3 m_Mirror56_g39838 = float3(1,1,-1);
				float3 localfloat3switch56_g39838 = float3switch56_g39838( m_switch56_g39838 , m_Default56_g39838 , m_Flip56_g39838 , m_Mirror56_g39838 );
				float3 switchResult58_g39838 = (((ase_vface>0)?(NORMAL_IN42_g39838):(( NORMAL_IN42_g39838 * localfloat3switch56_g39838 ))));
				int m_switch37_g39838 = _NormalMode;
				float3 m_Default37_g39838 = NORMAL_IN42_g39838;
				float3 m_Flip37_g39838 = ( NORMAL_IN42_g39838 * ase_vface );
				float3 break33_g39838 = NORMAL_IN42_g39838;
				float3 appendResult41_g39838 = (float3(break33_g39838.x , break33_g39838.y , ( break33_g39838.z * ase_vface )));
				float3 m_Mirror37_g39838 = appendResult41_g39838;
				float3 localfloat3switch37_g39838 = float3switch37_g39838( m_switch37_g39838 , m_Default37_g39838 , m_Flip37_g39838 , m_Mirror37_g39838 );
				float3 temp_output_1610_30_g39777 = localfloat3switch37_g39838;
				float3 NORMAL_OUT1491_g39777 = temp_output_1610_30_g39777;
				float3 temp_output_16_0_g39810 = NORMAL_OUT1491_g39777;
				float3 tanToWorld0 = float3( WorldTangent.x, WorldBiTangent.x, WorldNormal.x );
				float3 tanToWorld1 = float3( WorldTangent.y, WorldBiTangent.y, WorldNormal.y );
				float3 tanToWorld2 = float3( WorldTangent.z, WorldBiTangent.z, WorldNormal.z );
				float3 tanNormal134_g39810 = temp_output_16_0_g39810;
				float3 worldNormal134_g39810 = float3(dot(tanToWorld0,tanNormal134_g39810), dot(tanToWorld1,tanNormal134_g39810), dot(tanToWorld2,tanNormal134_g39810));
				float3 NORMAL_TANGENTSPACE135_g39810 = worldNormal134_g39810;
				float3 WorldPosition60_g39810 = NORMAL_TANGENTSPACE135_g39810;
				float3 localAdditionalLightsFlat60_g39810 = AdditionalLightsFlat( WorldPosition60_g39810 );
				float3 ADDITIONAL_LIGHT1212_g39777 = localAdditionalLightsFlat60_g39810;
				
				float4 MASK_B1377_g39777 = SAMPLE_TEXTURE2D( _MetallicGlossMap, sampler_trilinear_repeat, UV40_g39822 );
				
				int m_switch188_g39778 = _SmoothnessModeCutout;
				float3 tanNormal1608_g39777 = NORMAL_OUT1491_g39777;
				float3 worldNormal1608_g39777 = normalize( float3(dot(tanToWorld0,tanNormal1608_g39777), dot(tanToWorld1,tanNormal1608_g39777), dot(tanToWorld2,tanNormal1608_g39777)) );
				float3 temp_output_4_0_g39778 = worldNormal1608_g39777;
				float3 temp_output_178_0_g39778 = ddx( temp_output_4_0_g39778 );
				float dotResult195_g39778 = dot( temp_output_178_0_g39778 , temp_output_178_0_g39778 );
				float3 temp_output_175_0_g39778 = ddy( temp_output_4_0_g39778 );
				float dotResult201_g39778 = dot( temp_output_175_0_g39778 , temp_output_175_0_g39778 );
				float temp_output_196_0_g39778 = sqrt( saturate( ( ( _SmoothnessStrength * _SmoothnessStrength ) + min( ( ( _SmoothnessVariance * ( dotResult195_g39778 + dotResult201_g39778 ) ) * 2.0 ) , ( _SmoothnessThreshold * _SmoothnessThreshold ) ) ) ) );
				float ALBEDO_R172_g39777 = tex2DNode63_g39822.r;
				float4 temp_cast_15 = (ALBEDO_R172_g39777).xxxx;
				float4 temp_output_9_0_g39778 = temp_cast_15;
				float4 m_Standard188_g39778 = ( min( temp_output_196_0_g39778 , 0.5 ) * ( 1.0 - ( temp_output_9_0_g39778 * float4( 0.79,0,0,0 ) ) ) );
				float4 temp_output_187_0_g39778 = ( 1.0 - temp_output_9_0_g39778 );
				float4 m_Geometric188_g39778 = ( temp_output_196_0_g39778 * temp_output_187_0_g39778 );
				float ALBEDO_A926_g39777 = tex2DNode63_g39822.a;
				int m_switch1287_g39777 = _GlancingClipMode;
				float m_Off1287_g39777 = 1.0;
				float3 normalizeResult1294_g39777 = normalize( cross( ddx( WorldPosition ) , ddy( WorldPosition ) ) );
				float dotResult1301_g39777 = dot( WorldViewDirection , normalizeResult1294_g39777 );
				float temp_output_1321_0_g39777 = ( 1.0 - abs( dotResult1301_g39777 ) );
				float m_Active1287_g39777 = ( 1.0 - ( temp_output_1321_0_g39777 * temp_output_1321_0_g39777 ) );
				float localfloatswitch1287_g39777 = floatswitch1287_g39777( m_switch1287_g39777 , m_Off1287_g39777 , m_Active1287_g39777 );
				float OPACITY_OUTMASK1277_g39777 = localfloatswitch1287_g39777;
				float CLIP_OPACITY200_g39777 = ( ALBEDO_A926_g39777 * OPACITY_OUTMASK1277_g39777 );
				float4 temp_cast_16 = (CLIP_OPACITY200_g39777).xxxx;
				float4 m_GeometricFade188_g39778 = ( temp_output_196_0_g39778 * temp_output_187_0_g39778 * temp_cast_16 );
				int m_switch189_g39778 = _SmoothnessType;
				float4 MASK_G158_g39777 = SAMPLE_TEXTURE2D( _SmoothnessMap, sampler_trilinear_repeat, UV40_g39822 );
				float4 temp_output_83_0_g39778 = MASK_G158_g39777;
				float4 m_Smoothness189_g39778 = temp_output_83_0_g39778;
				float4 m_Roughness189_g39778 = ( 1.0 - temp_output_83_0_g39778 );
				float4 localfloat4switch189_g39778 = float4switch189_g39778( m_switch189_g39778 , m_Smoothness189_g39778 , m_Roughness189_g39778 );
				float4 m_Texture188_g39778 = ( temp_output_196_0_g39778 * localfloat4switch189_g39778 );
				float4 localfloat4switch188_g39778 = float4switch188_g39778( m_switch188_g39778 , m_Standard188_g39778 , m_Geometric188_g39778 , m_GeometricFade188_g39778 , m_Texture188_g39778 );
				float4 temp_output_1324_33_g39777 = localfloat4switch188_g39778;
				float4 color1225_g39777 = IsGammaSpace() ? float4(1,1,1,1) : float4(1,1,1,1);
				float4 MASK_R1378_g39777 = SAMPLE_TEXTURE2D( _OcclusionMap, sampler_trilinear_repeat, UV40_g39822 );
				float4 temp_cast_18 = (IN.ase_color.a).xxxx;
				float4 lerpResult28_g39777 = lerp( MASK_R1378_g39777 , temp_cast_18 , (float)_OcclusionSourceMode);
				float4 lerpResult7_g39777 = lerp( color1225_g39777 , lerpResult28_g39777 , _OcclusionStrengthAO);
				
				float AlphaCutoffBias1289_g39777 = _AlphaCutoffBias;
				
				int m_switch72_g39808 = _EnableTranslucency;
				float4 m_Off72_g39808 = float4( 0,0,0,0 );
				int m_switch38_g39808 = _MapInverted;
				float4 color71_g39808 = IsGammaSpace() ? float4(0,0,0,0) : float4(0,0,0,0);
				float4 tex2DNode27_g39808 = SAMPLE_TEXTURE2D( _TranslucencyMap, sampler_trilinear_repeat, OUT_UV213_g39777 );
				float4 lerpResult34_g39808 = lerp( color71_g39808 , tex2DNode27_g39808 , _TranslucencyThickness);
				float4 m_Default38_g39808 = lerpResult34_g39808;
				float temp_output_69_0_g39808 = ( 0.0 - 1.0 );
				float temp_output_22_0_g39808 = ( tex2DNode27_g39808.r - 1.0 );
				float lerpResult66_g39808 = lerp( ( temp_output_69_0_g39808 / temp_output_22_0_g39808 ) , ( temp_output_22_0_g39808 / temp_output_69_0_g39808 ) , ( 0.7 + _TranslucencyThickness ));
				float4 temp_cast_24 = (saturate( lerpResult66_g39808 )).xxxx;
				float4 m_Inverted38_g39808 = temp_cast_24;
				float4 localfloat4switch38_g39808 = float4switch( m_switch38_g39808 , m_Default38_g39808 , m_Inverted38_g39808 );
				float lerpResult30_g39808 = lerp( 0.0 , ( 5.0 - IN.ase_texcoord7.zw.x ) , _TranslucencyThickness);
				float4 temp_cast_25 = (lerpResult30_g39808).xxxx;
				float4 lerpResult46_g39808 = lerp( localfloat4switch38_g39808 , temp_cast_25 , (float)_TranslucencySource);
				float3 temp_output_90_0_g39808 = saturate( ( _MainLightColor.rgb * _MainLightColor.a ) );
				float4 m_Active72_g39808 = ( ( _TransmissionTint * _TransmissionTint.a * lerpResult46_g39808 ) * float4( temp_output_90_0_g39808 , 0.0 ) );
				float4 localfloat4switch72_g39808 = float4switch72_g39808( m_switch72_g39808 , m_Off72_g39808 , m_Active72_g39808 );
				
				int m_switch50_g39808 = _EnableTranslucency;
				float4 m_Off50_g39808 = float4( 0,0,0,0 );
				float4 m_Active50_g39808 = ( ( _TranslucencyTint * _TranslucencyTint.a * lerpResult46_g39808 ) * float4( temp_output_90_0_g39808 , 0.0 ) );
				float4 localfloat4switch50_g39808 = float4switch50_g39808( m_switch50_g39808 , m_Off50_g39808 , m_Active50_g39808 );
				
				float3 Albedo = ( localfloat4switch364_g39828 + float4(0,0,0,0) ).xyz;
				float3 Normal = ( switchResult58_g39838 + ADDITIONAL_LIGHT1212_g39777 );
				float3 Emission = 0;
				float3 Specular = 0.5;
				float Metallic = ( _MetallicStrength * MASK_B1377_g39777 ).r;
				float Smoothness = ( temp_output_1324_33_g39777 * lerpResult7_g39777 ).x;
				float Occlusion = saturate( lerpResult7_g39777 ).r;
				float Alpha = ( ALBEDO_A926_g39777 * OPACITY_OUTMASK1277_g39777 );
				float AlphaClipThreshold = AlphaCutoffBias1289_g39777;
				float AlphaClipThresholdShadow = _AlphaCutoffBiasShadow;
				float3 BakedGI = 0;
				float3 RefractionColor = 1;
				float RefractionIndex = 1;
				float3 Transmission = localfloat4switch72_g39808.xyz;
				float3 Translucency = localfloat4switch50_g39808.xyz;
				#ifdef ASE_DEPTH_WRITE_ON
				float DepthValue = 0;
				#endif
		...
		Pass
		{
						 ...
						 ) : SV_TARGET
			{
				...
				float2 appendResult168_g39777 = (float2(_TilingX , _TilingY));
				float2 appendResult167_g39777 = (float2(_OffsetX , _OffsetY));
				float2 texCoord2_g39816 = IN.ase_texcoord2.xy * appendResult168_g39777 + appendResult167_g39777;
				float2 appendResult6_g39816 = (float2(texCoord2_g39816.x , texCoord2_g39816.y));
				float2 OUT_UV213_g39777 = appendResult6_g39816;
				float2 UV40_g39822 = OUT_UV213_g39777;
				float4 tex2DNode63_g39822 = SAMPLE_TEXTURE2D( _MainTex, sampler_trilinear_repeat, UV40_g39822 );
				...
				float3 normalizeResult1294_g39777 = normalize( cross( ddx( WorldPosition ) , ddy( WorldPosition ) ) );
				float dotResult1301_g39777 = dot( ase_worldViewDir , normalizeResult1294_g39777 );
				float temp_output_1321_0_g39777 = ( 1.0 - abs( dotResult1301_g39777 ) );
				float m_Active1287_g39777 = ( 1.0 - ( temp_output_1321_0_g39777 * temp_output_1321_0_g39777 ) );
				float localfloatswitch1287_g39777 = floatswitch1287_g39777( m_switch1287_g39777 , m_Off1287_g39777 , m_Active1287_g39777 );
				float OPACITY_OUTMASK1277_g39777 = localfloatswitch1287_g39777;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float3 p1_g39829 = ( WorldPosition * _ColorShift_WorldSpaceNoiseShift );
				float localSimpleNoise3D1_g39829 = SimpleNoise3D( p1_g39829 );
				float4 transform374_g39828 = mul(GetObjectToWorldMatrix(),IN.ase_texcoord3);
				float m_WorldSpace168_g39828 = ( ( localSimpleNoise3D1_g39829 / _ColorShift_NoiseScale ) - ( ( (transform374_g39828).w - _ColorShift_WorldSpaceOffset ) / _ColorShift_WorldSpaceDistance ) );
				float m_VertexColor168_g39828 = ( IN.ase_color.g - 0.5 );
				float m_VertexNormal168_g39828 = ( IN.ase_normal.y - 0.5 );
				float localfloatswitch168_g39828 = floatswitch168_g39828( m_switch168_g39828 , m_Off168_g39828 , m_ObjectSpace168_g39828 , m_WorldSpace168_g39828 , m_VertexColor168_g39828 , m_VertexNormal168_g39828 );
				...
				float4 transform376_g39828 = mul(GetObjectToWorldMatrix(),float4( IN.ase_texcoord3.xyz , 0.0 ));
				float4 temp_output_337_0_g39828 = saturate( ( 1.0 - ( ( SAMPLE_TEXTURE2D( _ColorShift_MaskMap, sampler_trilinear_repeat, uv_ColorShift_MaskMap ) * transform376_g39828 ) / max( _ColorShift_MaskFuzziness , 1E-05 ) ) ) );
				float3 lerpResult314_g39828 = lerp( hsvTorgb126_g39828 , ALBEDO_IN136_g39828 , temp_output_337_0_g39828.rgb);
				float4 m_Active313_g39828 = float4( lerpResult314_g39828 , 0.0 );
				float3 lerpResult311_g39828 = lerp( ALBEDO_IN136_g39828 , hsvTorgb126_g39828 , temp_output_337_0_g39828.rgb);
...
/*ASEBEGIN
Version=18913
4;29.33333;1436;766;5489.4;4502.264;1.9;True;False
Node;AmplifyShaderEditor.CommentaryNode;2820;-3955.45,-3844.181;Inherit;False;379.5028;189.4201;;4;2849;2760;202;2758;DEBUG SETTINGS ;0,0,0,1;0;0
Node;AmplifyShaderEditor.CommentaryNode;2819;-3957.129,-4009.892;Inherit;False;179.1001;128.1001;;1;2848;GLOBAL SETTINGS ;0,0,0,1;0;0
Node;AmplifyShaderEditor.CommentaryNode;2871;-3934.061,-3286.948;Inherit;False;323.8669;119.7;DESF Common ASE Compile Shaders;1;2872;;0,0.2047877,1,1;0;0
Node;AmplifyShaderEditor.CommentaryNode;2861;-4630.494,-4226.279;Inherit;False;599.3186;531.5;URP TRANSMISSION TRANSLUCENCY ;7;2868;2867;2866;2865;2864;2863;2862;;0,0,0,1;0;0
Node;AmplifyShaderEditor.FunctionNode;2869;-4648.095,-3642.448;Inherit;False;DESF Module Wind;126;;3466;b135a554f7e4d0b41bba02c61b34ae74;11,938,0,2881,0,2454,1,2457,1,2432,1,2434,1,2433,1,2371,1,2752,0,2995,0,2878,0;0;2;FLOAT3;2190;FLOAT4;2970
Node;AmplifyShaderEditor.RangedFloatNode;2862;-4336.876,-3935.481;Inherit;False;Property;_URP_TranslucencyDirect;Translucency Direct ;123;0;Create;False;1;;0;0;True;0;False;0.9;0.763;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;2866;-4342.342,-4013.481;Inherit;False;Property;_URP_TranslucencyScattering;Translucency Scattering ;122;0;Create;False;1;;0;0;True;0;False;2;2.7;1;50;0;1;FLOAT;0
Node;AmplifyShaderEditor.IntNode;2758;-3952.076,-3733.627;Inherit;False;Property;_ColorMask;Color Mask Mode;1;1;[Enum];Create;False;1;;0;1;None,0,Alpha,1,Red,8,Green,4,Blue,2,RGB,14,RGBA,15;True;0;False;15;15;False;0;1;INT;0
Node;AmplifyShaderEditor.RangedFloatNode;202;-3947.471,-3810.078;Inherit;False;Constant;_MaskClipValue;Mask Clip Value;64;1;[HideInInspector];Create;True;1;;0;0;True;0;False;0.5;1;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;2867;-4336.276,-3786.081;Inherit;False;Property;_URP_TranslucencyShadow;Translucency Shadow ;125;0;Create;False;1;;0;0;True;0;False;0.5;0.437;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;2863;-4617.992,-4178.424;Inherit;False;Property;_URP_TransmissionShadow;Transmission Shadow;119;0;Create;False;1;;0;1;UnityEngine.Rendering.CompareFunction;True;1;Header(TRANSMISSION  URP);False;0.5;0.479;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;2849;-3765.048,-3736.398;Inherit;False;Property;_AlphatoCoverage;Alpha to Coverage;2;1;[Enum];Create;False;1;;1;Option1;0;1;Off,0,On,1;True;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;2868;-4331.876,-3861.481;Inherit;False;Property;_URP_TranslucencyAmbient;Translucency Ambient;124;0;Create;False;1;;0;0;True;0;False;0.1;0.573;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.FunctionNode;2872;-3919.161,-3248.648;Inherit;False;DESF Common ASE Compile Shaders;-1;;39840;b85b01c42ba8a8a448b731b68fc0dbd9;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;2865;-4339.875,-4096.48;Inherit;False;Property;_URP_TranslucencyNormalDistortion;Translucency Normal Distortion ;121;0;Create;False;1;;0;0;True;0;False;0.2735869;0.506;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.IntNode;2848;-3933.649,-3970.208;Inherit;False;Property;_CullMode;Cull Mode;3;2;[Header];[Enum];Create;True;1;GLOBAL SETTINGS;0;1;UnityEngine.Rendering.CullMode;True;0;False;0;2;False;0;1;INT;0
Node;AmplifyShaderEditor.FunctionNode;2873;-4405.351,-3643.053;Inherit;False;DESF Core Cutout;4;;39777;e0cdd7758f4404849b063afff4596424;26,1217,2,1311,2,1296,2,1156,2,1141,2,442,2,1509,0,1556,1,1557,1,1552,1,1309,1,1308,1,1333,0,1348,0,1596,0,1368,0,1569,0,96,2,1229,0,1236,0,830,0,1415,0,1399,0,1564,0,1566,0,1565,0;2;1234;FLOAT3;0,0,0;False;1274;FLOAT4;0,0,0,0;False;14;FLOAT4;38;FLOAT3;35;FLOAT3;1586;COLOR;37;FLOAT4;33;COLOR;34;FLOAT;441;FLOAT;46;FLOAT;814;FLOAT;552;FLOAT4;656;FLOAT4;657;FLOAT4;655;FLOAT3;1235
Node;AmplifyShaderEditor.RangedFloatNode;2864;-4340.993,-4176.695;Inherit;False;Property;_URP_TranslucencyStrength;Translucency Strength;120;0;Create;False;1;;0;0;True;1;Header(TRANSLUCENCY URP);False;1;20.3;1;50;0;1;FLOAT;0
Node;AmplifyShaderEditor.IntNode;2760;-3766.712,-3811.285;Inherit;False;Property;_ZWriteMode;ZWrite Mode;0;2;[Header];[Enum];Create;False;1;DEBUG SETTINGS;0;1;Off,0,On,1;True;0;False;1;1;False;0;1;INT;0
...
WireConnection;2873;1234;2869;2190
WireConnection;2873;1274;2869;2970
WireConnection;2841;0;2873;38
WireConnection;2841;1;2873;35
WireConnection;2841;3;2873;37
WireConnection;2841;4;2873;33
WireConnection;2841;5;2873;34
WireConnection;2841;6;2873;46
WireConnection;2841;7;2873;814
WireConnection;2841;16;2873;552
WireConnection;2841;14;2873;656
WireConnection;2841;15;2873;657
WireConnection;2841;8;2873;1235
ASEEND*/
//CHKSM=30C102B07893C34D2CF3D9AB187E8C34814CAC3A